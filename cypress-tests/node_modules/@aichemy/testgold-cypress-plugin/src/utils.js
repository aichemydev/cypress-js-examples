 ...
 * @returns {Promise<*>}
 */
export async function awaitableTimeout (ms) {
 ...
 */
export async function getSelectorId (selector, urlPath) {
  const selectorDigest = `${urlPath}|${selector}`
  const hasher = new Sha256()

  hasher.update(selectorDigest)

  let digestOutput = await hasher.digest()

  let textDigestArr = []

  for (let item of digestOutput) {
    textDigestArr.push(item.toString(16).padStart(2, '0'))
  }

  return textDigestArr.join('')
}
 ...
 */
export function serializeWebElement (
  $window,
  $elem,
  Cypress,
  cy,
 ...
 */
export function getInterceptorConfig (Cypress, getTestRunName = false) {
  const tgConfig = Cypress.config('testGold')
  let tgEnabled = false

  if (tgConfig) {
    tgEnabled = tgConfig['enabled']
    if (tgEnabled === undefined || tgEnabled === null) {
      tgEnabled = false
    }
  } else {
    // this reads CYPRESS_TG_ENABLED from the env
    const envTgEnabled = Cypress.env('TG_ENABLED')
  ...
  if (tgEnabled) {
    let tgToken = tgConfig['token']

    if (tgToken === undefined || tgToken === null) {
      // this reads CYPRESS_TG_TOKEN from the env
      const envTgToken = Cypress.env('TG_TOKEN')
    ...
    const decodedToken = JSON.parse(decodeBase64Unicode(tgToken))
    ...
    if (interceptorFilterEnabled === undefined || interceptorFilterEnabled === null) {
      interceptorFilterEnabled = false
    }
    ...
    if (interceptorFilterDisplayed === undefined || interceptorFilterDisplayed === null) {
      interceptorFilterDisplayed = false
    }
...
export function getPageProperties ($window, $document, Cypress) {
 ...
 * @returns {Promise<*>}
 */
export async function getTagMutationTable (tgConfig) {
 ...
 */
export async function startTestRequest (Cypress, tgConfig) {
  const url = `${tgConfig['walServerHost']}/testrequest/v1`

  let postData = new URLSearchParams({
    scriptType: 'javascript',
    testRunName: tgConfig['testRunName'],
 ...
 * @returns {Promise<string>}
 */
export async function finishTestRequest (requestId, tgConfig, Cypress) {
 ...
 * @param tgConfig
 */
export async function getInitialElementTag (selectorId, tgConfig) {
